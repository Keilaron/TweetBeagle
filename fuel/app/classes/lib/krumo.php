<?php
/**
* Lib_Krumo: Structured information display solution
*
* Lib_Krumo is a debugging tool (PHP5 only), which displays structured information
...
*
* @package Lib_Krumo
* @version $Id: class.Lib_Krumo.php 22 2007-12-02 07:38:18Z Mrasnika $
*/
...
/**
* Set the Lib_Krumo_DIR constant up with the absolute path to Lib_Krumo files. If it is
* not defined, include_path will be used. Set Lib_Krumo_DIR only if any other module
* or application has not already set it up.
*/
if (!defined('Lib_Krumo_DIR')) {
	define('Lib_Krumo_DIR', dirname(__FILE__) . DIRECTORY_SEPARATOR);
	}
...
*/
if (!defined('Lib_Krumo_TRUNCATE_LENGTH')) {
	define('Lib_Krumo_TRUNCATE_LENGTH', 50);
	}
...
* Lib_Krumo API
*
* This class stores the Lib_Krumo API for rendering and
...
Class Lib_Krumo {
	...
	*/
	Public Static Function backtrace() {
		...
		// render it
		//
		return Lib_Krumo::dump(debug_backtrace());
		}
...
This is a list of all currently declared classes.
		...
		<?php
		return Lib_Krumo::dump(get_declared_classes());
		}
...
This is a list of all currently declared interfaces.
		...
		<?php
		return Lib_Krumo::dump(get_declared_interfaces());
		}
...
This is a list of all currently included (or required) files.
		...
		<?php
		return Lib_Krumo::dump(get_included_files());
		}
...
This is a list of all currently declared functions.
		...
		<?php
		return Lib_Krumo::dump(get_defined_functions());
		}
...
This is a list of all currently declared constants (defines).
		...
		<?php
		return Lib_Krumo::dump(get_defined_constants());
		}
...
This is a list of all currently loaded PHP extensions.
		...
		<?php
		return Lib_Krumo::dump(get_loaded_extensions());
		}
...
This is a list of all HTTP request headers.
		...
		<?php
		return Lib_Krumo::dump(getAllHeaders());
		}
...
This is a list of the configuration settings read from <code><b><?php echo get_cfg_var('cfg_file_path');?></b></code>.
		...
		<?php
		return Lib_Krumo::dump(parse_ini_file(get_cfg_var('cfg_file_path'), true));
		}
...
This is a list of all your configuration settings.
		...
		<?php
		return Lib_Krumo::dump(ini_get_all());
		}
...
This is a list of the specified directories under your <code><b>include_path</b></code> option.
		...
		<?php
		return Lib_Krumo::dump(explode(PATH_SEPARATOR, ini_get('include_path')));
		}
...
This is a list of all the values from the <code><b>$_REQUEST</b></code> array.
		...
		<?php
		return Lib_Krumo::dump($_REQUEST);
		}
...
This is a list of all the values from the <code><b>$_SESSION</b></code> array.
		...
		<?php
		return Lib_Krumo::dump($_SESSION);
		}
...
This is a list of all the values from the <code><b><?php echo realpath($ini_file) ? realpath($ini_file) : $ini_file;?></b></code> INI file.
	...
	*/
	Public Static Function dump($data) {
		...
		$_ = debug_backtrace();
		while($d = array_pop($_)) {
			if ((strToLower($d['function']) == 'Lib_Krumo') || (strToLower(@$d['class']) == 'Lib_Krumo')) {
				break;
				}
			}
...
<div class="krumo-root">
	<ul class="krumo-node Lib_Krumo-first">
		<?php echo Lib_Krumo::_dump($data);?>
		<li class="krumo-footnote">
			<div class="krumo-version" style="white-space:nowrap;">
				<h6>Lib_Krumo version <?php echo Lib_Krumo::version();?></h6> | <a
					href="http://Lib_Krumo.sourceforge.net"
					target="_blank">http://Lib_Krumo.sourceforge.net</a>
			</div>
...
<?php
		// flee the hive
		//
		$_recursion_marker = Lib_Krumo::_marker();
		if ($hive =& Lib_Krumo::_hive($dummy)) {
		...
		if (is_array($data) && version_compare(PHP_VERSION, "5", "<")) {
			unset($GLOBALS[Lib_Krumo::_marker()]);
			}
		}
	...
	/**
	* Returns values from Lib_Krumo's configuration
	*
	* @param string $group
	* @param string $name
	...
	*/
	Private Static Function _css() {
		...
		$css = '';
		$skin = Lib_Krumo::_config('skin', 'selected', 'default');
		
		// custom selected skin ?
		//
		$_ = Lib_Krumo_DIR . "skins/{$skin}/skin.css";
		if ($fp = @fopen($_, 'r', 1)) {
		...
		if (!$css && ($skin != 'default')) {
			$skin = 'default';
			$_ = Lib_Krumo_DIR . "skins/default/skin.css";
			$css = join('', @file($_));
			}
		...
		if ($_css = $css != '') {
			...
			// fix the urls
			//
			$css_url = Lib_Krumo::_config('css', 'url') . "skins/{$skin}/";
			$css = preg_replace('~%url%~Uis', $css_url, $css);
			
			// the CSS
			//
			?>
<!-- Using Lib_Krumo Skin: <?php echo preg_replace('~^' . preg_quote(realpath(Lib_Krumo_DIR) . DIRECTORY_SEPARATOR) . '~Uis', '', realpath($_));?> -->
<style type="text/css">
<!--/**/
<?php echo $css?>
...
<!--//
<?php echo join(file(Lib_Krumo_DIR . "Lib_Krumo.js"));?>
...
<?php
	...
	*/
	Public Static Function enable() {
		return true === Lib_Krumo::_debug(true);
		}
	...
	*/
	Public Static Function disable() {
		return false === Lib_Krumo::_debug(false);
		}
	
	/**
	* Get\Set Lib_Krumo state: whether it is enabled or disabled
	*
	* @param boolean $state
	* @return boolean
	* @access private
	...
	*/
	Private Static Function _dump(&$data, $name='...') {
		...
		//
		if (is_object($data)) {
			return Lib_Krumo::_object($data, $name);
			}
		...
		if (is_array($data)) {
			...
			if (version_compare(PHP_VERSION, "5", "<")) {
				...
				//
				if (!isset($GLOBALS[Lib_Krumo::_marker()])) {
					$GLOBALS[Lib_Krumo::_marker()] = array();
					}
				if (!is_array($GLOBALS[Lib_Krumo::_marker()])) {
					$GLOBALS[Lib_Krumo::_marker()] = (array) $GLOBALS[Lib_Krumo::_marker()];
					}
				
				// extract ?
				//
				if (!empty($GLOBALS[Lib_Krumo::_marker()])) {
					$d = array_shift($GLOBALS[Lib_Krumo::_marker()]);
					if (is_array($d)) {
						$data = $d;
						}
					}
			...
			return Lib_Krumo::_array($data, $name);
			}

		// resource ?
		//
		if (is_resource($data)) {
			return Lib_Krumo::_resource($data, $name);
			}
		
		// scalar ?
		//
		if (is_string($data)) {
			return Lib_Krumo::_string($data, $name);
			}
		
		if (is_float($data)) {
			return Lib_Krumo::_float($data, $name);
			}

		if (is_integer($data)) {
			return Lib_Krumo::_integer($data, $name);
			}

		if (is_bool($data)) {
			return Lib_Krumo::_boolean($data, $name);
			}
		
		// null ?
		//
		if (is_null($data)) {
			return Lib_Krumo::_null($name);
			}
		}
...
<li class="krumo-child">
	<div class="krumo-element"
		onMouseOver="Lib_Krumo.over(this);"
		onMouseOut="Lib_Krumo.out(this);">
		
			<a class="krumo-name"><?php echo $name;?></a>
			(<em class="krumo-type Lib_Krumo-null">NULL</em>)
	</div>
</li>
<?php
		}
	...
	*/
	Private Static Function _marker() {
		...
		if (!isset($_recursion_marker)) {
			$_recursion_marker = uniqid('Lib_Krumo');
			}
	...
	*/
	Private Static Function _vars(&$data) {
		...
		//
		if ($_r > 0) {
			return Lib_Krumo::_recursion();
			}

		// stain it
		//
		Lib_Krumo::_hive($data);
...
<li class="krumo-child">
	...
	<div class="krumo-element<?php echo count($data) > 0 ? ' Lib_Krumo-expand' : '';?>"
		<?php if (count($data) > 0) {?> onClick="Lib_Krumo.toggle(this);"<?php } ?>
		onMouseOver="Lib_Krumo.over(this);"
		onMouseOut="Lib_Krumo.out(this);">
...
<?php
	...
	*/
	Private Static Function _string($data, $name) {
		...
		$_extra = false;
		$_ = $data;
		if (strLen($data) > Lib_Krumo_TRUNCATE_LENGTH) {
			$_ = substr($data, 0, Lib_Krumo_TRUNCATE_LENGTH - 3) . '...';
			$_extra = true;
			}
?>
<li class="krumo-child">

	<div class="krumo-element<?php echo $_extra ? ' Lib_Krumo-expand' : '';?>"
		<?php if ($_extra) {?> onClick="Lib_Krumo.toggle(this);"<?php } ?>
		onMouseOver="Lib_Krumo.over(this);"
		onMouseOut="Lib_Krumo.out(this);">
